...
QList<int> AbsRandNoise::GenerateNoise()
{
	...
	unsigned int amountOfNoisedPixels = 0;
	QList<int> listOfNoise;

	if ( true == m_needToNoise )
	{
		amountOfNoisedPixels = m_pixelsToNoise;
	}
	else
	{
		amountOfNoisedPixels = ( m_imgToNoise.width() * m_imgToNoise.height() ) - m_pixelsToNoise;
	}

	for (unsigned int i = 0; i < amountOfNoisedPixels; i++ )
	{
